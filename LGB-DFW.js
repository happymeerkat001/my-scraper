...
const DFW_RADIUS_COUNTIES = new Set([
  'ANDERSON COUNTY',
  'BOSQUE COUNTY',
  'BROWN COUNTY',
  'CLAY COUNTY',
  ...
  'LAMAR COUNTY',
  'LIMESTONE COUNTY',
  'MCLENNAN COUNTY',     // borderline but valuable
  'MONTAGUE COUNTY',
  'NAVARRO COUNTY',
  'PALO PINTO COUNTY',
  'PARKER COUNTY',
...
// → Counties whose seats fall roughly inside a three-hour drive from DFW; only these are kept downstream.

const filterToDfwRadius = (names) => [...new Set(names)].filter(name => DFW_RADIUS_COUNTIES.has(name));
...
async function getAllCounties() {                  // Listen > Start
  try {                                            // Run > Plan > Check (error guard)
    ...
    const filtered = filterToDfwRadius(names);     // Run > Execute > Work
    if (filtered.length) {                        // Run > Plan > Check
      console.log(`📍 Found ${filtered.length} counties within ~3h of DFW via sale_counties endpoint`); // Render > Internal
  ...
  try {                                           // Run > Plan > Check
    ...
    const filtered = filterToDfwRadius(names);     // Run > Execute > Work
    console.log(`📍 Found ${filtered.length} counties within ~3h of DFW via /counties/`); // Render > Internal
  ...
  try {                                           // Run > Plan > Check
    ...
    while (true) {                                // Run > Execute > Flow (loop)
      ...
      data.results.forEach(p => {                 // Run > Execute > Flow
        ...
        if (DFW_RADIUS_COUNTIES.has(name)) set.add(name); // Run > Plan > Choose + Execute
      });
    ...
    console.log(`📍 Found ${arr.length} counties within ~3h of DFW by scanning property_sales`); // Render > Internal
